...
def scan_directory(
    ...
    if gitignore_path.is_file():
        ...
        logger.info(f"Loaded .gitignore in '{directory}'")
    ...
    for i, entry in enumerate(entries):
        ...
        if entry in config.excluded_files or path.suffix == ".exe":
            logger.info(f"Excluded '{entry}'")
            continue

        if is_ignored(relative_path, current_ignored_paths, logger):
